...
#############################################################################
'''
Precise and fast Fermi-Dirac and inverse Fermi-Dirac integrals
...
cdef inline double fdm9h(double phi):
    ...
    if(phi < -2e0):
        ...
        fd=exp_phi*(0.270088205852269109e0 \
        -exp_phi*(98457.373700674016e0 \
        ...
        +s))))))))))*(phi+1.82715356570827096e0)	# care for zero point
    elif(phi < 2e0):
        t=0.5e0*phi
        ...
        -t)))))))))*(phi-0.557104412737456300e0)	# care for zero point
    elif(phi < 5e0):
        ...
        -t)))))))))*(phi-3.79898975457237574e0)	# care for zero point
    elif(phi < 10e0):
        t=0.2e0*phi-1e0
        ...
        w=1e0/(phi*phi)
        s=1e0-1600e0*w
        fd=factor/(sqrt(phi)*phi*phi*phi)*(1e0 \
...
cdef inline double fdm7h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(-0.945308720482941881e0 \
        +exp_phi*(22521.6817696480830e0 \
        ...
        +s))))))))*(phi+0.731435761340666539e0)	# care for zero point
    elif(phi < 2e0):
        t=0.5e0*phi
        ...
        -t)))))))))*(phi-2.59355543171509487e0)	# care for zero point
    elif(phi < 10e0):
        t=0.2e0*phi-1e0
        ...
        w=1e0/(phi*phi)
        s=1e0-1600e0*w
        fd=factor/(sqrt(phi)*phi*phi)*(1e0 \
...
cdef inline double fdm5h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(2.36327180120735470e0 \
        -exp_phi*(55891.891151428516e0 \
        ...
        +t))))))))*(phi-1.10894923342229868e0)	# care for zero point
    elif(phi < 5e0):
        t=0.3333333333333333333e0*(phi-2e0)
...
cdef inline double fdm3h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(-3.54490770181103205e0 \
        +exp_phi*(82737.595643818605e0 \
...
cdef inline double fdm1h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(1.77245385090551603e0 \
        -exp_phi*(40641.4537510284430e0 \
...
cdef inline double fd1h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(0.886226925452758014e0 \
...
cdef inline double fd3h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(1.32934038817913702e0 \
        -exp_phi*(1346.14119046566636e0 \
        ...
        w=1e0/(phi*phi)
        s=1e0-1600e0*w
        fd=phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd5h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(3.32335097044784255e0 \
        -exp_phi*(3004.41138112148735e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd7h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(11.6317283965674489e0 \
        -exp_phi*(9371.8868848378853e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd9h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(52.3427777845535202e0 \
        -exp_phi*(37544.8291647986266e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd11h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(287.885277815044361e0 \
        -exp_phi*(183706.438585548927e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd13h(double phi):
    ...
    if(phi < -2e0):
        ...
        t=exp_phi*7.38905609893065023e0
        s=1e0-t
        fd=exp_phi*(1871.25430579778835e0 \
        -exp_phi*(10.2714824358192133e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd15h(double phi):
    ...
    if(phi < -2e0):
        exp_phi=exp(phi)
        t=exp_phi*7.38905609893065023e0
        fd=exp_phi*(14034.4072934834126e0 \
        -exp_phi*(171704.258234536602e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd17h(double phi):
    ...
    if(phi < -2e0):
        ...
        t=exp_phi*7.38905609893065023e0
        s=1e0-t
        fd=exp_phi*(119292.461994609007e0 \
        -exp_phi*(164.281538211493590e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd19h(double phi):
    ...
    if(phi < -2e0):
        ...
        t=exp_phi*7.38905609893065023e0
        s=1e0-t
        fd=exp_phi*(1.13327838894878557e6 \
        -exp_phi*(781.077395263087427e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double fd21h(double phi):
    ...
    if(phi < -2e0):
        ...
        t=exp_phi*7.38905609893065023e0
        s=1e0-t
        fd=exp_phi*(1.18994230839622485e7 \
        -exp_phi*(4103.25503466860405e0 \
        ...
        w=1e0/(phi*phi)
        t=1600e0*w
        fd=phi*phi*phi*phi*phi*phi*phi*phi*phi*phi*phi*sqrt(phi)*factor*(1e0 \
...
cdef inline double ifd1h(double nu):
    ...
    '''
    cdef double v,s,t,w,y,z
    if(nu < 1.17683303804380831e0):
        t=nu*0.849738210666018375e0
        z=t*(156377.8333056294e0 \
        +t*(48177.5705898287e0 \
        +t*(5847.07218383812e0 \
        +t*(335.3978079672194e0 \
        +t*7.84411868029912e0 \
        ))))/(117762.02905535089e0 \
        +t*(-19007.26938370368e0 \
        +t*(1376.2936928453140e0 \
        +t*(-54.11372698481717e0 \
        +t))))
        y=log(z)
    elif(nu < 3.82993088157949761e0):
        t=0.376917874490198033e0*nu \
        -0.443569407329314587e0
        y=(489.140447310410217e0 \
        +t*(5335.07269317261966e0 \
        +t*(20169.0736140442509e0 \
        +t*(35247.8115595510907e0 \
        +t*(30462.3668614714761e0 \
        +t*(12567.9032426128967e0 \
        +t*(2131.86789357398657e0 \
        +t*93.6520172085419439e0 \
        )))))))/(656.826207643060606e0 \
        +t*(4274.82831051941605e0 \
        +t*(10555.7581310151498e0 \
        +t*(12341.8742094611883e0 \
        +t*(6949.18854413197094e0 \
        +t*(1692.19650634194002e0 \
        +t*(129.221772991589751e0 \
        +t)))))))
    elif(nu < 13.3854493161866553e0):
        t=0.104651569335924949e0*nu \
        -0.400808277205416960e0
        y=(1019.84886406642351e0 \
        +t*(9440.18255003922075e0 \
        +t*(33947.6616363762463e0 \
        +t*(60256.7280980542786e0 \
        +t*(55243.0045063055787e0 \
        +t*(24769.8354802210838e0 \
        +t*(4511.77288617668292e0 \
        +t*211.432806336150141e0 \
        )))))))/(350.502070353586442e0 \
        +t*(2531.06296201234050e0 \
        +t*(6939.09850659439245e0 \
        +t*(9005.40197972396592e0 \
        +t*(5606.73612994134056e0 \
        +t*(1488.76634564005075e0 \
        +t*(121.537028889412581e0 \
        +t)))))))
    elif(nu < 53.2408277860982205e0):
        t=0.0250907164450825724e0*nu \
        -0.335850513282463787e0
        y=(11885.8779398399498e0 \
        +t*(113220.250825178799e0 \
        +t*(408524.373881197840e0 \
        +t*(695674.357483475952e0 \
        +t*(569389.917088505552e0 \
        +t*(206433.082013681440e0 \
        +t*(27307.2535671974100e0 \
        +t*824.430826794730740e0 \
        )))))))/(1634.40491220861182e0 \
        +t*(12218.1158551884025e0 \
        +t*(32911.7869957793233e0 \
        +t*(38934.6963039399331e0 \
        +t*(20038.8358438225823e0 \
        +t*(3949.48380897796954e0 \
        +t*(215.607404890995706e0 \
        +t)))))))
    elif(nu < 188.411871723022843e0):
        t=0.00739803415638806339e0*nu \
        -0.393877462475929313e0
        y=(11730.7011190435638e0 \
        +t*(99421.7455796633651e0 \
        +t*(327706.968910706902e0 \
        +t*(530425.668016563224e0 \
        +t*(438631.900516555072e0 \
        +t*(175322.855662315845e0 \
        +t*(28701.9605988813884e0 \
        +t*1258.20914464286403e0 \
        )))))))/(634.080470383026173e0 \
        +t*(4295.63159860265838e0 \
        +t*(10868.5260668911946e0 \
        +t*(12781.6871997977069e0 \
        +t*(7093.80732100760563e0 \
        +t*(1675.06417056300026e0 \
        +t*(125.750901817759662e0 \
        +t)))))))
    else:
        v=nu**(-4.e0/3.e0)
        ...
        +t*(420368.911157160874e0 \
        +t*(1689.69475714536117e0 \
        +t)))/(s*(6088.08350831295857e0 \
        +t*(221.445236759466761e0 \
        +t*0.718216708695397737e0 \
        )))
        y=sqrt(w)
    return y
...
cdef void vifd1h(np.ndarray[double] nu, np.ndarray[double] out):
    '''
    Vectorized form of ifd1h.
    '''
    cdef int imax = nu.shape[0]
    assert imax == out.shape[0]
    cdef int i
    for i in range(imax):
        out[i] = ifd1h(nu[i])
...
def ipara(nu):
    if isinstance(nu, numpy.ndarray):
        phi = numpy.empty_like(nu, dtype=numpy.double)
        vifd1h(nu, phi)
        return phi
    else:
        return ifd1h(nu)
